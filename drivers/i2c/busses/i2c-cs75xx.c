...
typedef volatile union {
  struct {
    u_int32_t biwdone	      :  1 ; /* bits 0:0 */
    u_int32_t rsrvd1	       :  2 ;
    u_int32_t ack_in	       :  1 ; /* bits 3:3 */
    u_int32_t write		:  1 ; /* bits 4:4 */
    u_int32_t read		 :  1 ; /* bits 5:5 */
    u_int32_t stop		 :  1 ; /* bits 6:6 */
    u_int32_t start		:  1 ; /* bits 7:7 */
    u_int32_t rsrvd2	       : 24 ;
  } bf ;
  u_int32_t     wrd ;
} PER_BIW_CTRL_t;
...
typedef volatile union {
  struct {
    u_int32_t core_en	      :  1 ; /* bits 0:0 */
    u_int32_t rsrvd1	       : 15 ;
    u_int32_t prer		 : 16 ; /* bits 31:16 */
  } bf ;
  u_int32_t     wrd ;
} PER_BIW_CFG_t;
...
static int cs75xx_i2c_execute_msg(struct cs75xx_i2c_data *drv_data, struct i2c_msg *msg)
{
	spin_lock(&drv_data->lock);
 ...
 *****************************************************************************
 */
static int __init cs75xx_i2c_map_regs(struct platform_device *pd,
...
static void cs75xx_i2c_unmap_regs(struct cs75xx_i2c_data *drv_data)
{
...
static struct cs75xx_i2c_pdata* __init cs75xx_i2c_of_parse(struct platform_device *pdev)
{
	struct cs75xx_i2c_pdata	*pdata;
	...
	clk = of_clk_get(np, 0);
	if (IS_ERR(clk)) {
		dev_err(&pdev->dev, "could not get clock resource\n");
		goto l_free;
	}
	pdata->freq_rcl = clk_get_rate(clk);

	res = of_property_read_u32(np, "clock-frequency", &ultmp);
	if (res) {
		dev_err(&pdev->dev, "could not get property of I2C bus frequency.\n");
		goto l_free;
	}
	pdata->freq_scl = ultmp;

	res = of_property_read_u32(np, "timeout", &ultmp);
	if (res)
		ultmp = DEFAULT_TIMEOUT;
	pdata->timeout = ultmp;

	res = of_property_read_u32(np, "reries", &ultmp);
	if (res)
		ultmp = DEFAULT_RETRIES;
	pdata->retries = ultmp;
...
static int __init cs75xx_i2c_probe(struct platform_device *pd)
{
	struct cs75xx_i2c_data *drv_data;
	...
	if (cs75xx_i2c_map_regs(pd, drv_data)) {
		rc = -ENODEV;
		dev_err(&pd->dev, "I/O resource mapping failed.\n");
		goto exit_kfree;
	}
...
static int __exit cs75xx_i2c_remove(struct platform_device *dev)
{
...
static struct platform_driver cs75xx_i2c_driver = {
	...
	.remove	= __exit_p(cs75xx_i2c_remove),
	.driver	= {
		.owner	= THIS_MODULE,
...
static int __init cs75xx_i2c_init(void)
{
	return platform_driver_register(&cs75xx_i2c_driver);
}

static void __exit cs75xx_i2c_exit(void)
{
