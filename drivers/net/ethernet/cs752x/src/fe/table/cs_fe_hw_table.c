		...
		bool parity)
{
	...
	FE_WRITEL(tbl_data0.wrd, FETOP_FE_L3TBL_DATA0);
	FE_WRITEL(tbl_data1.wrd, FETOP_FE_L3TBL_DATA1);
	FE_WRITEL(tbl_data2.wrd, FETOP_FE_L3TBL_DATA2);
	FE_WRITEL(tbl_data3.wrd, FETOP_FE_L3TBL_DATA3);
	FE_WRITEL(tbl_data4.wrd, FETOP_FE_L3TBL_DATA4);
	FE_WRITEL(tbl_access.wrd, FETOP_FE_L3TBL_ACCESS);

	/* wait write complete */
	try_loop = 0;
	do {
		tbl_access.wrd = FE_READL(FETOP_FE_L3TBL_ACCESS);
		...
		bool *p_parity)
{
	...
	tbl_data0.wrd = FE_READL(FETOP_FE_L3TBL_DATA0);
	tbl_data1.wrd = FE_READL(FETOP_FE_L3TBL_DATA1);
	tbl_data2.wrd = FE_READL(FETOP_FE_L3TBL_DATA2);
	tbl_data3.wrd = FE_READL(FETOP_FE_L3TBL_DATA3);
	tbl_data4.wrd = FE_READL(FETOP_FE_L3TBL_DATA4);
		...
		unsigned int idx, unsigned int field, __u32 *p_value)
{
	...
	FE_WRITEL(reg, access_reg_loc);

	/* wait read complete */
	try_loop = 0;
	do {
		reg = FE_READL(access_reg_loc);
...
int cs_fe_hw_table_clear_entry(cs_fe_hw_table_e table_type, unsigned int idx)
{
	...
	for (i = 0; i < data_reg_num; i++)
		FE_WRITEL(0, table_head[0].start_reg - (i << 2));
		...
		unsigned int idx, unsigned int data_cnt, __u32 *p_data_array)
{
	...
	for (i = 0; i < data_cnt; i++)
		FE_WRITEL(p_data_array[i], access_reg_loc +
				((data_reg_cnt - i) << 2));
