...
int cs_lpm_init(u8 debug_mode)
{
	...
	do {
		lpm_mem_status.wrd = FE_READL(FETOP_LPM_MEM_STATUS);
		try_loop++;
		mdelay(1);
	...
	FE_WRITEL(lpm_mem_init.wrd, FETOP_LPM_MEM_INIT);
	do {
		lpm_mem_status.wrd = FE_READL(FETOP_LPM_MEM_STATUS);
		try_loop++;
		mdelay(1);
	...
	lpm_config.wrd = FE_READL(FETOP_LPM_LPM_CONFIG);
	lpm_config.bf.sdb_en = (debug_mode == 1)? 0:1;
	lpm_config.bf.active_table = 1;
	FE_WRITEL(lpm_config.wrd, FETOP_LPM_LPM_CONFIG);
...
int cs_lpm_get_entity_by_idx(u8 index, lpm_entity_info_t *entity)
{
	...
	if (entity->is_v6){ /* check if it's V6 */
		...
		if (entity->index>(MAX_LPM_ENTRY_CNT4-MAX_LPM_ENTRY_CNT6/2-1)){ 	/* use upper table */
			...
			FE_WRITEL(hc_access.wrd,(table==2)? FETOP_LPM_HC_TABLE1_UPPER_REGF_ACCESS:FETOP_LPM_HC_TABLE0_UPPER_REGF_ACCESS);
			/* Wait read complete*/
			while((hc_access.bf.ACCESS==1)&&(i<LPM_INIT_TIMEOUT))
				hc_access.wrd = FE_READL((table==2)? FETOP_LPM_HC_TABLE1_UPPER_REGF_ACCESS:FETOP_LPM_HC_TABLE0_UPPER_REGF_ACCESS);
			if (i>=LPM_INIT_TIMEOUT)
				return FE_TABLE_EOPNOTSUPP;  /* ACCESS can't complete */
			hc_data0.wrd = FE_READL((table==2)? FETOP_LPM_HC_TABLE1_UPPER_REGF_DATA0:FETOP_LPM_HC_TABLE0_UPPER_REGF_DATA0);
			hc_data1.wrd = FE_READL((table==2)? FETOP_LPM_HC_TABLE1_UPPER_REGF_DATA0:FETOP_LPM_HC_TABLE0_UPPER_REGF_DATA1);
			...
			FE_WRITEL(hc_access.wrd,(table==2)? FETOP_LPM_HC_TABLE1_LOWER_REGF_ACCESS:FETOP_LPM_HC_TABLE0_LOWER_REGF_ACCESS);
 			/* Wait read complete*/
			while((hc_access.bf.ACCESS==1)&&(i<LPM_INIT_TIMEOUT))
				hc_access.wrd = FE_READL((table==2)? FETOP_LPM_HC_TABLE1_LOWER_REGF_ACCESS:FETOP_LPM_HC_TABLE0_LOWER_REGF_ACCESS);
			if (i>=LPM_INIT_TIMEOUT)
				return FE_TABLE_EOPNOTSUPP;  /* ACCESS can't complete */
			hc_data0.wrd = FE_READL((table==2)? FETOP_LPM_HC_TABLE1_LOWER_REGF_DATA0:FETOP_LPM_HC_TABLE0_LOWER_REGF_DATA0);
			hc_data1.wrd = FE_READL((table==2)? FETOP_LPM_HC_TABLE1_LOWER_REGF_DATA0:FETOP_LPM_HC_TABLE0_LOWER_REGF_DATA1);
			entity->hit_count = hc_data1.wrd;
			entity->hit_count = entity->hit_count<<32;
			entity->hit_count |= hc_data0.wrd;
			lpm_table_info.lpm_entries[index].hit_count = entity->hit_count;
  ...
  printk("%s: IPv4 index=%d, entity->index=%d\n", __func__, index, entity->index);
		if (entity->index>(MAX_LPM_ENTRY_CNT4/2-1)){ 	/* use upper table */
			...
			switch (entity->index%4){
				...
				case 1:
					hc_data0.wrd = FE_READL((table==2)? FETOP_LPM_HC_TABLE1_UPPER_REGF_DATA0:FETOP_LPM_HC_TABLE0_UPPER_REGF_DATA0);
					hc_data1.wrd = FE_READL((table==2)? FETOP_LPM_HC_TABLE1_UPPER_REGF_DATA1:FETOP_LPM_HC_TABLE0_UPPER_REGF_DATA1);
					entity->hit_count = (hc_data1.bf.up_hit_cnt01<<5)|hc_data0.bf.up_hit_cnt01;
					lpm_table_info.lpm_entries[index].hit_count = entity->hit_count;
					break;
				case 2:
					hc_data1.wrd = FE_READL((table==2)? FETOP_LPM_HC_TABLE1_UPPER_REGF_DATA1:FETOP_LPM_HC_TABLE0_UPPER_REGF_DATA1);
					hc_data2.wrd = FE_READL((table==2)? FETOP_LPM_HC_TABLE1_UPPER_REGF_DATA2:FETOP_LPM_HC_TABLE0_UPPER_REGF_DATA2);
					entity->hit_count = (hc_data2.bf.up_hit_cnt02<<10)|hc_data1.bf.up_hit_cnt02;
					lpm_table_info.lpm_entries[index].hit_count = entity->hit_count;
					break;
				case 3:
					hc_data2.wrd = FE_READL((table==2)? FETOP_LPM_HC_TABLE1_UPPER_REGF_DATA2:FETOP_LPM_HC_TABLE0_UPPER_REGF_DATA2);
					hc_data3.wrd = FE_READL((table==2)? FETOP_LPM_HC_TABLE1_UPPER_REGF_DATA3:FETOP_LPM_HC_TABLE0_UPPER_REGF_DATA3);
			...
			switch (entity->index%4){
				...
				case 1:
					hc_data0.wrd = FE_READL((table==2)? FETOP_LPM_HC_TABLE1_LOWER_REGF_DATA0:FETOP_LPM_HC_TABLE0_LOWER_REGF_DATA0);
					hc_data1.wrd = FE_READL((table==2)? FETOP_LPM_HC_TABLE1_LOWER_REGF_DATA1:FETOP_LPM_HC_TABLE0_LOWER_REGF_DATA1);
					entity->hit_count = (hc_data1.bf.up_hit_cnt01<<5)|hc_data0.bf.up_hit_cnt01;
					lpm_table_info.lpm_entries[index].hit_count = entity->hit_count;
					break;
				case 2:
					hc_data1.wrd = FE_READL((table==2)? FETOP_LPM_HC_TABLE1_LOWER_REGF_DATA1:FETOP_LPM_HC_TABLE0_LOWER_REGF_DATA1);
					hc_data2.wrd = FE_READL((table==2)? FETOP_LPM_HC_TABLE1_LOWER_REGF_DATA2:FETOP_LPM_HC_TABLE0_LOWER_REGF_DATA2);
					entity->hit_count = (hc_data2.bf.up_hit_cnt02<<10)|hc_data1.bf.up_hit_cnt02;
					lpm_table_info.lpm_entries[index].hit_count = entity->hit_count;
					break;
				case 3:
					hc_data2.wrd = FE_READL((table==2)? FETOP_LPM_HC_TABLE1_LOWER_REGF_DATA2:FETOP_LPM_HC_TABLE0_LOWER_REGF_DATA2);
					hc_data3.wrd = FE_READL((table==2)? FETOP_LPM_HC_TABLE1_LOWER_REGF_DATA3:FETOP_LPM_HC_TABLE0_LOWER_REGF_DATA3);
					entity->hit_count = (hc_data3.bf.up_hit_cnt03<<15)|hc_data2.bf.up_hit_cnt03;
					lpm_table_info.lpm_entries[index].hit_count = entity->hit_count;
					break;
				default:
	...
	if (entity->is_v6) {
		...
		if (entity->index > ((MAX_LPM_ENTRY_CNT4-MAX_LPM_ENTRY_CNT6 / 2) - 1)) {	/* Upper table */
			...
			is_upper = 1;
	   		...
	   		FE_WRITEL(data0.wrd, (table == 2)? FETOP_LPM_LPM_TABLE1_UPPER_REGF_DATA0 : FETOP_LPM_LPM_TABLE0_UPPER_REGF_DATA0);
			FE_WRITEL(data1.wrd, (table == 2)? FETOP_LPM_LPM_TABLE1_UPPER_REGF_DATA1 : FETOP_LPM_LPM_TABLE0_UPPER_REGF_DATA1);
			FE_WRITEL(data3.wrd, (table == 2)? FETOP_LPM_LPM_TABLE1_UPPER_REGF_DATA3 : FETOP_LPM_LPM_TABLE0_UPPER_REGF_DATA3);
			FE_WRITEL(data4.wrd, (table == 2)? FETOP_LPM_LPM_TABLE1_UPPER_REGF_DATA4 : FETOP_LPM_LPM_TABLE0_UPPER_REGF_DATA4);
			FE_WRITEL(data5.wrd, (table == 2)? FETOP_LPM_LPM_TABLE1_UPPER_REGF_DATA5 : FETOP_LPM_LPM_TABLE0_UPPER_REGF_DATA5);
			FE_WRITEL(data6.wrd, (table == 2)? FETOP_LPM_LPM_TABLE1_UPPER_REGF_DATA6 : FETOP_LPM_LPM_TABLE0_UPPER_REGF_DATA6);
			FE_WRITEL(data7.wrd, (table == 2)? FETOP_LPM_LPM_TABLE1_UPPER_REGF_DATA7 : FETOP_LPM_LPM_TABLE0_UPPER_REGF_DATA7);
			FE_WRITEL(data8.wrd, (table == 2)? FETOP_LPM_LPM_TABLE1_UPPER_REGF_DATA8 : FETOP_LPM_LPM_TABLE0_UPPER_REGF_DATA8);
		} else {	/* Lower table */
			is_upper = 0;
...
#endif
			FE_WRITEL(data0.wrd,(table==2)? FETOP_LPM_LPM_TABLE1_LOWER_REGF_DATA0:FETOP_LPM_LPM_TABLE0_LOWER_REGF_DATA0);
			FE_WRITEL(data1.wrd,(table==2)? FETOP_LPM_LPM_TABLE1_LOWER_REGF_DATA1:FETOP_LPM_LPM_TABLE0_LOWER_REGF_DATA1);
			FE_WRITEL(data3.wrd,(table==2)? FETOP_LPM_LPM_TABLE1_LOWER_REGF_DATA3:FETOP_LPM_LPM_TABLE0_LOWER_REGF_DATA3);
			FE_WRITEL(data4.wrd,(table==2)? FETOP_LPM_LPM_TABLE1_LOWER_REGF_DATA4:FETOP_LPM_LPM_TABLE0_LOWER_REGF_DATA4);
			FE_WRITEL(data5.wrd,(table==2)? FETOP_LPM_LPM_TABLE1_LOWER_REGF_DATA5:FETOP_LPM_LPM_TABLE0_LOWER_REGF_DATA5);
			FE_WRITEL(data6.wrd,(table==2)? FETOP_LPM_LPM_TABLE1_LOWER_REGF_DATA6:FETOP_LPM_LPM_TABLE0_LOWER_REGF_DATA6);
			FE_WRITEL(data7.wrd,(table==2)? FETOP_LPM_LPM_TABLE1_LOWER_REGF_DATA7:FETOP_LPM_LPM_TABLE0_LOWER_REGF_DATA7);
			FE_WRITEL(data8.wrd,(table==2)? FETOP_LPM_LPM_TABLE1_LOWER_REGF_DATA8:FETOP_LPM_LPM_TABLE0_LOWER_REGF_DATA8);
		}
	} else {
		if (entity->index > (MAX_LPM_ENTRY_CNT4 / 2 - 1)) {	/* Upper table */
			...
			access.bf.w_rdn=0;
			access.bf.ACCESS=1;
			FE_WRITEL(access.wrd,(table==2)? FETOP_LPM_LPM_TABLE1_UPPER_REGF_ACCESS:FETOP_LPM_LPM_TABLE0_UPPER_REGF_ACCESS );
			/* Check if access complete */
			while((i<LPM_INIT_TIMEOUT) && (access.bf.ACCESS==1))
				access.wrd = FE_READL((table==2)? FETOP_LPM_LPM_TABLE1_UPPER_REGF_ACCESS:FETOP_LPM_LPM_TABLE0_UPPER_REGF_ACCESS);
			if (i>=LPM_INIT_TIMEOUT)
				return FE_TABLE_EOPNOTSUPP;

			data0.wrd = FE_READL((table==2)? FETOP_LPM_LPM_TABLE1_UPPER_REGF_DATA0:FETOP_LPM_LPM_TABLE0_UPPER_REGF_DATA0);
			data1.wrd = FE_READL((table==2)? FETOP_LPM_LPM_TABLE1_UPPER_REGF_DATA1:FETOP_LPM_LPM_TABLE0_UPPER_REGF_DATA1);
			data2.wrd = FE_READL((table==2)? FETOP_LPM_LPM_TABLE1_UPPER_REGF_DATA2:FETOP_LPM_LPM_TABLE0_UPPER_REGF_DATA2);
			data3.wrd = FE_READL((table==2)? FETOP_LPM_LPM_TABLE1_UPPER_REGF_DATA3:FETOP_LPM_LPM_TABLE0_UPPER_REGF_DATA3);
			data4.wrd = FE_READL((table==2)? FETOP_LPM_LPM_TABLE1_UPPER_REGF_DATA4:FETOP_LPM_LPM_TABLE0_UPPER_REGF_DATA4);
			data5.wrd = FE_READL((table==2)? FETOP_LPM_LPM_TABLE1_UPPER_REGF_DATA5:FETOP_LPM_LPM_TABLE0_UPPER_REGF_DATA5);
			data6.wrd = FE_READL((table==2)? FETOP_LPM_LPM_TABLE1_UPPER_REGF_DATA6:FETOP_LPM_LPM_TABLE0_UPPER_REGF_DATA6);
			data7.wrd = FE_READL((table==2)? FETOP_LPM_LPM_TABLE1_UPPER_REGF_DATA7:FETOP_LPM_LPM_TABLE0_UPPER_REGF_DATA7);
			data8.wrd = FE_READL((table==2)? FETOP_LPM_LPM_TABLE1_UPPER_REGF_DATA8:FETOP_LPM_LPM_TABLE0_UPPER_REGF_DATA8);
			...
			case 2:
				...
				FE_WRITEL(data1.wrd,(table==2)? FETOP_LPM_LPM_TABLE1_UPPER_REGF_DATA1:FETOP_LPM_LPM_TABLE0_UPPER_REGF_DATA1);
				FE_WRITEL(data2.wrd,(table==2)? FETOP_LPM_LPM_TABLE1_UPPER_REGF_DATA2:FETOP_LPM_LPM_TABLE0_UPPER_REGF_DATA2);
				FE_WRITEL(data5.wrd,(table==2)? FETOP_LPM_LPM_TABLE1_UPPER_REGF_DATA5:FETOP_LPM_LPM_TABLE0_UPPER_REGF_DATA5);
				FE_WRITEL(data6.wrd,(table==2)? FETOP_LPM_LPM_TABLE1_UPPER_REGF_DATA6:FETOP_LPM_LPM_TABLE0_UPPER_REGF_DATA6);
				break;
			case 3:
			...
			access.bf.w_rdn = 0;
			access.bf.ACCESS = 1;
			FE_WRITEL(access.wrd, (table==2)? FETOP_LPM_LPM_TABLE1_LOWER_REGF_ACCESS:FETOP_LPM_LPM_TABLE0_LOWER_REGF_ACCESS );
			/* Check if access complete */
			while ((i < LPM_INIT_TIMEOUT) && (access.bf.ACCESS == 1))
				access.wrd = FE_READL((table==2)? FETOP_LPM_LPM_TABLE1_LOWER_REGF_ACCESS:FETOP_LPM_LPM_TABLE0_LOWER_REGF_ACCESS);
			if (i >= LPM_INIT_TIMEOUT)
				return FE_TABLE_EOPNOTSUPP;

			data0.wrd = FE_READL((table==2)? FETOP_LPM_LPM_TABLE1_LOWER_REGF_DATA0:FETOP_LPM_LPM_TABLE0_LOWER_REGF_DATA0);
			data1.wrd = FE_READL((table==2)? FETOP_LPM_LPM_TABLE1_LOWER_REGF_DATA1:FETOP_LPM_LPM_TABLE0_LOWER_REGF_DATA1);
			data2.wrd = FE_READL((table==2)? FETOP_LPM_LPM_TABLE1_LOWER_REGF_DATA2:FETOP_LPM_LPM_TABLE0_LOWER_REGF_DATA2);
			data3.wrd = FE_READL((table==2)? FETOP_LPM_LPM_TABLE1_LOWER_REGF_DATA3:FETOP_LPM_LPM_TABLE0_LOWER_REGF_DATA3);
			data4.wrd = FE_READL((table==2)? FETOP_LPM_LPM_TABLE1_LOWER_REGF_DATA4:FETOP_LPM_LPM_TABLE0_LOWER_REGF_DATA4);
			data5.wrd = FE_READL((table==2)? FETOP_LPM_LPM_TABLE1_LOWER_REGF_DATA5:FETOP_LPM_LPM_TABLE0_LOWER_REGF_DATA5);
			data6.wrd = FE_READL((table==2)? FETOP_LPM_LPM_TABLE1_LOWER_REGF_DATA6:FETOP_LPM_LPM_TABLE0_LOWER_REGF_DATA6);
			data7.wrd = FE_READL((table==2)? FETOP_LPM_LPM_TABLE1_LOWER_REGF_DATA7:FETOP_LPM_LPM_TABLE0_LOWER_REGF_DATA7);
			data8.wrd = FE_READL((table==2)? FETOP_LPM_LPM_TABLE1_LOWER_REGF_DATA8:FETOP_LPM_LPM_TABLE0_LOWER_REGF_DATA8);
			...
			case 2:
				...
				FE_WRITEL(data1.wrd,(table==2)? FETOP_LPM_LPM_TABLE1_LOWER_REGF_DATA1:FETOP_LPM_LPM_TABLE0_LOWER_REGF_DATA1);
				FE_WRITEL(data2.wrd,(table==2)? FETOP_LPM_LPM_TABLE1_LOWER_REGF_DATA2:FETOP_LPM_LPM_TABLE0_LOWER_REGF_DATA2);
				FE_WRITEL(data5.wrd,(table==2)? FETOP_LPM_LPM_TABLE1_LOWER_REGF_DATA5:FETOP_LPM_LPM_TABLE0_LOWER_REGF_DATA5);
				FE_WRITEL(data6.wrd,(table==2)? FETOP_LPM_LPM_TABLE1_LOWER_REGF_DATA6:FETOP_LPM_LPM_TABLE0_LOWER_REGF_DATA6);
				break;
			case 3:
...
int cs_lpm_active_table(u8 table)
{
	...
	do{
		lpm_status.wrd = FE_READL(FETOP_LPM_LPM_STATUS);
