...
#include "cs75xx_qos.h"        //BUG#39672: 2. QoS     Bug#42574
...
void __iomem*	g_iobase_global;
void __iomem*	g_iobase_ni;
void __iomem*	g_iobase_mdio;
void __iomem*	g_iobase_dma_lso;
void __iomem*	g_iobase_fe;
...
int ni_mdio_write(int phy_addr, int reg_addr, u16 value)
{
	...
	cfg.wrd = MDIO_READL(PER_MDIO_CFG);
	/* Set the MDIO mode as Manual */
	cfg.bf.mdio_manual = 1;
	MDIO_WRITEL(cfg.wrd, PER_MDIO_CFG);
	addr.wrd = MDIO_READL(PER_MDIO_ADDR);
	...
	MDIO_WRITEL(addr.wrd, PER_MDIO_ADDR);

	ctrl.wrd = MDIO_READL(PER_MDIO_CTRL);
	wrdata.wrd = MDIO_READL(PER_MDIO_WRDATA);
	wrdata.bf.mdio_wrdata = value;
	MDIO_WRITEL(wrdata.wrd, PER_MDIO_WRDATA);

	ctrl.bf.mdiodone = 0;
	/* Start the MDIO Operation */
	ctrl.bf.mdiostart = 1;
	MDIO_WRITEL(ctrl.wrd, PER_MDIO_CTRL);
...
int ni_mdio_read(int phy_addr, int reg_addr)
{
	...
	value = MDIO_READL(PER_MDIO_RDDATA) & 0xffff;
						 ...
						 int qid, int cnt)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		if (ni_rx_noncache){    //QM_ACP is not enabled if use external DDR
			...
			dma_map_single(&dev->dev, phys_to_virt(skb->head_pa) + 224, 32, DMA_FROM_DEVICE);
		}
#endif
		...
		QM_WRITEL(phy_addr, QM_CPU_PATH_LINUX0_SDRAM_ADDR + (qid * 4));
...
#if defined(CONFIG_NET_POLL_CONTROLLER) || defined(CONFIG_GENERIC_IRQ)
/* Handles NE interrupts. */
static irqreturn_t ni_generic_interrupt(int irq, void *dev_instance)
{
	u32 status;

	status = GLOBAL_READL(GLOBAL_NETWORK_ENGINE_INTERRUPT_0);
	GLOBAL_WRITEL(status, GLOBAL_NETWORK_ENGINE_INTERRUPT_0);
				   ...
				   int budget, struct napi_struct *napi)
{
	...
	hw_wr_ptr = (NI_READL(NI_TOP_NI_CPUXRAM_CPU_STA_RX_0 + (instance * 24)));
	hw_wr_ptr &= HW_WR_PTR;
	...
	while ((next_link != hw_wr_ptr) && (done < budget)) {
		...
		if (next_link == hw_wr_ptr) {
			...
			u32 hw_wr_ptr_cur;
			hw_wr_ptr_cur = (NI_READL(NI_TOP_NI_CPUXRAM_CPU_STA_RX_0 + (instance * 24)));
...
static void rx_enable_interrupt( struct work_struct *work)
{
	...
	NI_WRITEL(1, NI_TOP_NI_CPUXRAM_RXPKT_0_INTENABLE_0 + (task->instance << 3));
}
...
static void cs_ni_reset(void)
{
	...
	/* Apply resets */
	reg_val = NI_READL(NI_TOP_NI_INTF_RST_CONFIG);
	reg_val = reg_val | (RESET_NI);	/* ->1 */
	NI_WRITEL(reg_val, NI_TOP_NI_INTF_RST_CONFIG);
	/* Remove resets */
	reg_val = NI_READL(NI_TOP_NI_INTF_RST_CONFIG);
...
int cs_ni_set_port_calendar(u16 table_address, u8 pspid_ts)
{
	...
	NI_WRITE_BITS(rx_port_cal_data.wrd, rx_port_cal_data_mask.wrd, NI_TOP_NI_RX_PORT_CAL_DATA);
	...
	NI_WRITE_BITS(rx_port_cal_acc.wrd, rx_port_cal_acc_mask.wrd, NI_TOP_NI_RX_PORT_CAL_ACCESS);

	do {
		access = (NI_READL(NI_TOP_NI_TX_VOQ_LKUP_ACCESS) & 0x80000000);
		udelay(1);
...
int ni_get_port_calendar(u16 table_address)
{
	...
	do {
		access = (NI_READL(NI_TOP_NI_RX_PORT_CAL_ACCESS) & 0x80000000);
		udelay(1);
	} while (access == 0x80000000);

	/* read data */
	cal_data = NI_READL(NI_TOP_NI_RX_PORT_CAL_DATA) & 0x7;
...
static void cs_ni_init_xram_mem(void)
{
	...
	for (i = 0; i <= XRAM_RX_INSTANCE; i++)
		NI_WRITEL(0, NI_TOP_NI_CPUXRAM_ADRCFG_RX_0 + (i * 4));

	for (i = 0; i < XRAM_TX_INSTANCE; i++)
		NI_WRITEL(0, NI_TOP_NI_CPUXRAM_ADRCFG_TX_0 + (i * 4));
	...
	for (i = 0; i <= XRAM_RX_INSTANCE; i++) {
		...
		NI_WRITE_BITS(xram_rx_addr.wrd, xram_rx_addr_mask.wrd, NI_TOP_NI_CPUXRAM_ADRCFG_RX_0 + (i * 4));
	}
	...
	for (i = 0; i < XRAM_TX_INSTANCE; i++) {
		...
		NI_WRITE_BITS(xram_tx_addr.wrd, xram_tx_addr_mask.wrd, NI_TOP_NI_CPUXRAM_ADRCFG_TX_0 + (i * 4));
	}
}
		...
		xram_direction_t xram_direction)
{
	...
	case XRAM_DIRECTION_TX:
		...
		NI_WRITE_BITS(ni_xram_cfg.wrd, ni_xram_cfg_mask.wrd, NI_TOP_NI_CPUXRAM_CFG);
		break;
...
static void cs_init_tx_dma_lso(void)
{
	...
	DMA_LSO_WRITE_BITS(rxdma_control.wrd, rxdma_control_mask.wrd, DMA_DMA_LSO_RXDMA_CONTROL);
	...
	DMA_LSO_WRITE_BITS(txdma_control.wrd, txdma_control_mask.wrd, DMA_DMA_LSO_TXDMA_CONTROL);
	...
	for (i = 0; i < NI_DMA_LSO_TXQ_NUM; i++)
		DMA_LSO_WRITE_BITS(txq0_control.wrd, txq0_control_mask.wrd, DMA_DMA_LSO_TXQ0_CONTROL
					+ (i * 4));
	...
	if( cs_acp_enable & CS75XX_ACP_ENABLE_NI){
		...
		DMA_LSO_WRITEL((txq_paddr[i] & DMA_BASE_MASK) | NI_DMA_LSO_TXDESC, dma_tx_base_addr);
		dma_tx_base_addr += 4;
		rptr_addr += 8;
		wptr_addr += 8;
	}
	...
	DMA_LSO_WRITEL(0xFFFC0000, DMA_D_AXI_READ_TIMEOUT_THRESHOLD);
	DMA_LSO_WRITEL(0xFFFC0000, DMA_D_AXI_WRITE_TIMEOUT_THRESHOLD);
	...
	DMA_LSO_WRITE_BITS(axi_config.wrd, axi_config_mask.wrd, DMA_D_AXI_CONFIG);

	DMA_LSO_WRITEL(0xFFFFFFFF, DMA_D_AXI_READ_CHANNEL_0_3_DRR_WEIGHT);
	DMA_LSO_WRITEL(0x0000FFFF, DMA_D_AXI_READ_CHANNEL_4_7_DRR_WEIGHT);

	DMA_LSO_WRITEL(0xFFFFFFFF, DMA_D_AXI_WRITE_CHANNEL_0_3_DRR_WEIGHT);
	DMA_LSO_WRITEL(0x0000FFFF, DMA_D_AXI_WRITE_CHANNEL_4_7_DRR_WEIGHT);
#ifdef CONFIG_CS752X_VIRTUAL_NI_DBLTAG
	...
	DMA_LSO_WRITE_BITS(vlan_tag_type0.wrd, vlan_tag_type0_mask.wrd, DMA_DMA_LSO_VLAN_TAG_TYPE0);
#endif
}
...
static void cs_ni_set_voq_map(u16 voq_number, u8 voq_did, u8 disable_crc)
{
	...
	NI_WRITE_BITS(voq_lkup_data0.wrd, voq_lkup_data0_mask.wrd, NI_TOP_NI_TX_VOQ_LKUP_DATA0);
	NI_WRITEL(0, NI_TOP_NI_TX_VOQ_LKUP_DATA1);
	...
	NI_WRITE_BITS(voq_lkup_access.wrd, voq_lkup_access_mask.wrd, NI_TOP_NI_TX_VOQ_LKUP_ACCESS);
	do {
		access = (NI_READL(NI_TOP_NI_TX_VOQ_LKUP_ACCESS) & 0x80000000);
		udelay(1);
	} while (access == 0x80000000);
...
void cs_ni_set_mc_table(u8 mc_index, u16 mc_vec)
{
	...
	NI_WRITE_BITS(mc_lkup_data.wrd, mc_lkup_data_mask.wrd, NI_TOP_NI_MC_INDX_LKUP_DATA);
	...
	NI_WRITE_BITS(mc_lkup_access.wrd, mc_lkup_access_mask.wrd, NI_TOP_NI_MC_INDX_LKUP_ACCESS);
	do {
		access = (NI_READL(NI_TOP_NI_MC_INDX_LKUP_ACCESS) & 0x80000000);
		udelay(1);
	} while (access == 0x80000000);
}
...
void cs_ni_get_mc_table(u8 mc_index)
{
	...
	mc_table_data = NI_READL(NI_TOP_NI_MC_INDX_LKUP_DATA) & 0xFFFF;
...
static void cs_ne_init_cfg(void)
{
	 ...
	 */
	NI_WRITEL(0x0829c0d8, NI_TOP_NI_SCH_BP_THLD_ETH);
#else
	/* ASIC use default value */
	NI_WRITEL(0x082a00f8, NI_TOP_NI_SCH_BP_THLD_ETH);
#endif

	/* Hold all macs in reset */
	for (i = 0; i < GE_PORT_NUM; i++)
		NI_WRITEL(0xc0100800, NI_TOP_NI_ETH_MAC_CONFIG0_0 + (i * 12));
	...
	if (ne_initialized == 0) {
		...
		MDIO_WRITE_BITS(mdio_cfg.wrd, mdio_cfg_mask.wrd, PER_MDIO_CFG);
	}
	...
	GLOBAL_WRITE_BITS(driver_strength.wrd, driver_strength.wrd,
	...
	NI_WRITE_BITS(rx_agg_config.wrd, rx_agg_config_mask.wrd, NI_TOP_NI_RX_AGG_CONFIG);
	...
	rx_ctl_cfg1_mask.wrd = 0;
	for (i = 0; i < XRAM_RX_INSTANCE; i++)
		NI_WRITE_BITS(rx_ctl_cfg1.wrd, rx_ctl_cfg1_mask.wrd, NI_TOP_NI_RX_CNTRL_CONFIG1_0 +
				(i * 8));
#else
	for (i = 0; i < XRAM_RX_INSTANCE; i++)
		NI_WRITEL(0, NI_TOP_NI_RX_CNTRL_CONFIG1_0 + (i * 8));
	 ...
	 * 7 - Mirror */
	for (i = 3; i < 6; i++)
		NI_WRITE_BITS(rx_ctl_cfg0.wrd, rx_ctl_cfg0_mask.wrd, NI_TOP_NI_RX_CNTRL_CONFIG0_0 +
				 (i * 8));
#endif /* BYPASS_FE */
	...
	for (i = 0; i < 8; i++) {
		...
		NI_WRITEL(sch_bp_reg.wrd, NI_TOP_NI_CPUXRAM_SCH_BP_CFG_0 + i * 4);
	}
	...
	NI_WRITE_BITS(pkt_len_cfg.wrd, pkt_len_cfg_mask.wrd, NI_TOP_NI_PKT_LEN_CONFIG);
	...
	NI_WRITE_BITS(mgmt_pt_cfg.wrd, mgmt_pt_cfg_mask.wrd, NI_TOP_NI_ETH_MGMT_PT_CONFIG);
	...
	NI_WRITE_BITS(misc_config.wrd, misc_config_mask.wrd, NI_TOP_NI_MISC_CONFIG);
#ifdef BYPASS_FE
	/* Bypass bypass_pr, bypass_pe for debugging QM linux mode */
	FE_WRITEL(0x800010d8, FETOP_FE_PE_CONFIG);
#endif /* BYPASS_FE */
...
#define PE_SEND_ALL_CPU_HDR 0x3FF
	FE_WRITEL(PE_SEND_ALL_CPU_HDR | PE_SET_MIN_CYCLE_PPARSER, FETOP_FE_PE_CONFIG_1);
...
{
	int i;
	NI_WRITEL(0, NI_TOP_NI_CPUXRAM_TXPKT_INTENABLE_0);
...
static inline void cs_ni_enable_interrupts(void)
{
	...
	for (i = 0; i <= XRAM_INST_MAX; i++)
		NI_WRITEL(ni_private_data.intr_cpuxram_rxpkt_mask, NI_TOP_NI_CPUXRAM_RXPKT_0_INTENABLE_0
				 + (i * 8));
}
...
static inline void cs_ni_reset_tx_ring(void)
{
	...
	for (i = 0; i < NI_DMA_LSO_TXQ_NUM; i++) {
		...
		DMA_LSO_WRITEL(rptr_reg.bits32, swtxq->wptr_reg);
...
static int cs_mfe_set_mac_address(struct net_device *dev)
{
	 ...
	 * packets.
	 */
	NI_WRITEL(low_mac, NI_TOP_NI_MAC_ADDR0);

	mac_addr1.wrd = 0;
	mac_addr1_mask.wrd = 0;
	mac_addr1.bf.mac_addr1 = mac_fifth_byte;
	mac_addr1_mask.bf.mac_addr1 = 0xFF;
	NI_WRITE_BITS(mac_addr1.wrd, mac_addr1_mask.wrd, NI_TOP_NI_MAC_ADDR1);
	...
	switch (tp->port_id) {
	case GE_PORT0:
		NI_WRITE_BITS(mac_config2.wrd, mac_config2_mask.wrd, NI_TOP_NI_ETH_MAC_CONFIG2_0);
		break;
	case GE_PORT1:
		NI_WRITE_BITS(mac_config2.wrd, mac_config2_mask.wrd, NI_TOP_NI_ETH_MAC_CONFIG2_0 + 12);
		break;
	case GE_PORT2:
		NI_WRITE_BITS(mac_config2.wrd, mac_config2_mask.wrd, NI_TOP_NI_ETH_MAC_CONFIG2_0 + 24);
		break;
	}
...
static int cs_ni_set_flowctrl(struct net_device *dev)
{
	...
	switch (tp->port_id) {
	case GE_PORT0:
		NI_WRITE_BITS(mac_config0.wrd, mac_config0_mask.wrd, NI_TOP_NI_ETH_MAC_CONFIG0_0);
		break;
	case GE_PORT1:
		NI_WRITE_BITS(mac_config0.wrd, mac_config0_mask.wrd, NI_TOP_NI_ETH_MAC_CONFIG0_0 + 12);
		break;
	case GE_PORT2:
		NI_WRITE_BITS(mac_config0.wrd, mac_config0_mask.wrd, NI_TOP_NI_ETH_MAC_CONFIG0_0 + 24);
		break;
	}
...
static void cs_ne_global_interrupt(void)
{
	...
	GLOBAL_WRITE_BITS(global_interrupt.wrd, global_interrupt_mask.wrd,
...
static void cs_ni_init_interrupt_cfg(void)
{
	...
	for (i = 0; i < XRAM_INST_MAX; i++) {	/* not only 3, max is 8 */
		...
		NI_WRITEL(ni->intr_port_mask[i], NI_TOP_NI_PORT_0_INTENABLE_0 + (i * 8));
	}
	...
	NI_WRITEL(ni->intr_mask, NI_TOP_NI_INTENABLE_0);
	NI_WRITEL(ni->intr_rxfifo_mask, NI_TOP_NI_RXFIFO_INTENABLE_0);
	NI_WRITEL(ni->intr_txem_mask, NI_TOP_NI_TXEM_INTENABLE_0);

	NI_WRITEL(ni->intr_cpuxram_cnt_mask, NI_TOP_NI_CPUXRAM_CNTR_INTENABLE_0);
	/* Keep Xram interrupt disabled till device is registered */
	NI_WRITEL(ni->intr_cpuxram_err_mask, NI_TOP_NI_CPUXRAM_ERR_INTENABLE_0);
#endif

#ifndef CONFIG_INTR_COALESCING
	NI_WRITEL(ni->intr_cpuxram_rxpkt_mask, NI_TOP_NI_CPUXRAM_RXPKT_INTENABLE_0);
#endif
	...
	QM_WRITEL(ni->intr_qm_mask, QM_INTENABLE_0);

	/* Enable SCH interrupt */
	ni->intr_sch_mask = 0x1F; /* enable all SCH interrupt for debug */
	SCH_WRITEL(ni->intr_sch_mask, SCH_INTENABLE_0);
#endif
#endif
	...
	DMA_LSO_WRITEL(ni->intr_sch_mask, DMA_DMA_LSO_DMA_LSO_INTENABLE_0);

	/* enable all LSO interrupt for debug */
	//ni->intr_dma_lso_desc_mask = 0x3;
	ni->intr_dma_lso_desc_mask = 0x0;
	DMA_LSO_WRITEL(ni->intr_sch_mask, DMA_DMA_LSO_DESC_INTENABLE);

	for (i = 0; i < NI_DMA_LSO_RXQ_NUM; i++) {
		ni->intr_lso_rx_mask[i] = 0x3F;
		DMA_LSO_WRITEL(ni->intr_lso_rx_mask[i], DMA_DMA_LSO_RXQ0_INTENABLE + (i * 8));
	}

	for (i = 0; i < NI_DMA_LSO_TXQ_NUM; i++) {
		ni->intr_lso_rx_mask[i] = 0xF;
		DMA_LSO_WRITEL(ni->intr_lso_rx_mask[i], DMA_DMA_LSO_TXQ0_INTENABLE + (i * 8));
	}

	ni->intr_dma_lso_bmc_mask = 0x1;
	DMA_LSO_WRITEL(ni->intr_dma_lso_bmc_mask, DMA_DMA_LSO_BMC0_INTENABLE);
	...
	NI_WRITEL(reg.wrd, NI_TOP_NI_CPUXRAM_INT_COLSC_CFG_0);
#endif
}
...
static void ni_set_mac_tx_rx(u8 port, u8 flag)
{
	...
	NI_WRITE_BITS(config0.wrd, config0_mask.wrd, config_addr);
}
...
void cs_ni_flow_control(mac_info_t *tp, u8 rx_tx, u8 flag)
{
	...
	NI_WRITE_BITS(mac_status.wrd, mac_status_mask.wrd, config_addr);
...
void cs_ni_set_mac_speed_duplex(mac_info_t *tp, int mac_interface)
{
	...
	NI_WRITE_BITS(eth_cfg1.wrd, eth_cfg1_mask.wrd, NI_TOP_NI_ETH_INT_CONFIG1);
	NI_WRITE_BITS(eth_cfg2.wrd, eth_cfg2_mask.wrd, NI_TOP_NI_ETH_INT_CONFIG2);
...
void cs_ni_set_eth_cfg(mac_info_t * tp, int config)
{
	...
	if (config == NI_CONFIG_1)
		NI_WRITE_BITS(eth_config1.wrd, eth_config1_mask.wrd, NI_TOP_NI_ETH_INT_CONFIG1);
	if (config == NI_CONFIG_2)
		NI_WRITE_BITS(eth_config2.wrd, eth_config2_mask.wrd, NI_TOP_NI_ETH_INT_CONFIG2);
...
static void cs_ni_init_port(struct net_device *dev)
{
	...
	*/
	...
	switch (tp->port_id) {
	case GE_PORT0:
		NI_WRITE_BITS(rx_cntrl_config0.wrd, rx_cntrl_config0_mask.wrd, NI_TOP_NI_RX_CNTRL_CONFIG0_0);
		break;
	case GE_PORT1:
		NI_WRITE_BITS(rx_cntrl_config0.wrd, rx_cntrl_config0_mask.wrd, NI_TOP_NI_RX_CNTRL_CONFIG0_0 + 8);
		break;
	case GE_PORT2:
		NI_WRITE_BITS(rx_cntrl_config0.wrd, rx_cntrl_config0_mask.wrd, NI_TOP_NI_RX_CNTRL_CONFIG0_0 + 16);
		break;
	default:
		break;
	}
...
static irqreturn_t cs_ni_tx_interrupt(int irq, void *dev_instance)
{
	...
	status = DMA_LSO_READL(DMA_DMA_LSO_TXQ0_INTERRUPT + tx_qid * 8);
	if (status != 0) {
		DMA_LSO_WRITEL(0, DMA_DMA_LSO_TXQ0_INTENABLE + tx_qid * 8);
		DMA_LSO_WRITEL(status, DMA_DMA_LSO_TXQ0_INTERRUPT + tx_qid * 8);
		dev = get_dev_from_lso_txqid(tx_qid);
		cs_dma_tx_complete(tx_qid, dev);
		DMA_LSO_WRITEL(0x7, DMA_DMA_LSO_TXQ0_INTENABLE + tx_qid * 8);
	}
...
static irqreturn_t cs_ni_rx_interrupt(int irq, void *dev_instance)
{
	...
	NI_WRITEL(status, NI_TOP_NI_CPUXRAM_RXPKT_0_INTERRUPT_0 + (i * 8));
	NI_WRITEL(0, NI_TOP_NI_CPUXRAM_RXPKT_0_INTENABLE_0 + i * 8);
...
void cs_ni_set_rx_mode(struct net_device *dev)
{
	...
	spin_lock_irqsave(&tp->lock, flags);
	NI_WRITE_BITS(rx_mode.wrd, rx_mode_mask.wrd, NI_TOP_NI_CPUXRAM_CFG);
	NI_WRITE_BITS(misc_cfg.wrd, misc_cfg_mask.wrd, NI_TOP_NI_MISC_CONFIG);
	spin_unlock_irqrestore(&tp->lock, flags);
}
...
#define L4RxChkSum	(1 << 8)
	if (data) {
		val = FE_READL(FETOP_FE_PRSR_CFG_0) & ~L4RxChkSum;
	...
	FE_WRITEL(val, FETOP_FE_PRSR_CFG_0);
		...
		u8 port_id, u8 counter_id)
{
	...
	NI_WRITE_BITS(mib_access.wrd, mib_access_mask.wrd, reg);

	do {
		access_executed = (NI_READL(reg) & 0x80000000);
		udelay(10);
	} while (access_executed == 1);

	/* FIXME: need verify. which one is the top. */
	val_bottom = NI_READL(reg + 8);
	val_top = NI_READL(reg + 4);
...
static int __init cs_ni_init_module_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < GE_PORT_NUM; i++)
		NI_WRITEL(0x00100800, NI_TOP_NI_ETH_MAC_CONFIG0_0 + (i * 12));
	...
	if (tp->rx_checksum == CS_ENABLE) {
		...
		FE_WRITE_BITS(fe_prsr_cfg0.wrd, fe_prsr_cfg0_mask.wrd, FETOP_FE_PRSR_CFG_0);
