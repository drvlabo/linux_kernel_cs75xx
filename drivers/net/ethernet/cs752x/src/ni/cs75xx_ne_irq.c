...
#define DBG(x) {if (cs_ni_debug == DBG_IRQ) x;}
#else
#define DBG(x) {}
#endif
#endif
...
const static struct cs_ne_irq_info cs_ne_tmtop_irq_info = {
	...
	.intr_status = (TM_TC_BM_STATUS | TM_TC_POL_STATUS),

	//.intr_enable = (TM_TC_BM_ENABLE | TM_TC_POL_ENABLE | TM_TC_PM_ENABLE),
	.intr_enable = (TM_TC_BM_ENABLE | TM_TC_POL_ENABLE),
...
const static struct cs_ne_irq_info cs_ne_schcnfg_irq_info[4] = {
	{
		.module_name = "SCH_CNFG 0",
		...
		.read_reg	= SCH_READL,
		.write_reg	= SCH_WRITEL,
	},
	{
		.module_name = "SCH_CNFG 1",
		.intr_status = (SCH_CNFG_PARERR_VOQ_STATUS(0) |
				SCH_CNFG_PARERR_VOQ_STATUS(1) |
				SCH_CNFG_PARERR_VOQ_STATUS(2) |
				SCH_CNFG_PARERR_VOQ_STATUS(3) |
		...
		// need some trick here, four shaper parity err interrupts should use
		// different interrupt enable registers.
		.intr_e_reg  = SCH_SHAPER_PARITY_ERR_INTENABLE0_0 + 8,
	...
	},
	{
		...
		// need some trick here, four shaper parity err interrupts should use
		// different interrupt enable registers.
		.intr_e_reg  = SCH_SHAPER_PARITY_ERR_INTENABLE0_0 + 16,
	...
	},
	{
		...
		// need some trick here, four shaper parity err interrupts should use
		// different interrupt enable registers.
		.intr_e_reg  = SCH_SHAPER_PARITY_ERR_INTENABLE0_0 + 24,
...
const static struct cs_ne_irq_info cs_ne_schtop_irq_info = {
	...
	.sub_module[1] = &cs_ne_schcnfg_irq_info[0],
	.sub_module[2] = &cs_ne_schcnfg_irq_info[1],
	.sub_module[3] = &cs_ne_schcnfg_irq_info[2],
	.sub_module[4] = &cs_ne_schcnfg_irq_info[3],
...
const static struct cs_ne_irq_info cs_ne_ni_port_info[8] = {
	{
		.module_name = "NI_PORT 0",
	...
	},
	{
		...
		// trick for different ports?
		.intr_e_reg  = NI_TOP_NI_PORT_0_INTENABLE_0 + 8,
	...
	},
	{
		...
		// trick for different ports?
		.intr_e_reg  = NI_TOP_NI_PORT_0_INTENABLE_0 + 16,
	...
	},
	{
		...
		// trick for different ports?
		.intr_e_reg  = NI_TOP_NI_PORT_0_INTENABLE_0 + 24,
	...
	},
	{
		...
		// trick for different ports?
		.intr_e_reg  = NI_TOP_NI_PORT_0_INTENABLE_0 + 32,
	...
	},
	{
		...
		// trick for different ports?
		.intr_e_reg  = NI_TOP_NI_PORT_0_INTENABLE_0 + 40,
	...
	},
	{
		...
		// trick for different ports?
		.intr_e_reg  = NI_TOP_NI_PORT_0_INTENABLE_0 + 48,
	...
	},
	{
		...
		// trick for different ports?
		.intr_e_reg  = NI_TOP_NI_PORT_0_INTENABLE_0 + 56,
...
const static struct cs_ne_irq_info cs_ne_ni_pc_info[3] = {
	{
		.module_name = "NI_PC 0",
	...
	},
	{
		...
		.intr_e_reg  = NI_TOP_NI_PC0_INTENABLE_0 + 8,
	...
	},
	{
		...
		.intr_e_reg  = NI_TOP_NI_PC0_INTENABLE_0 + 16,
...
const static struct cs_ne_irq_info cs_ne_ni_irq_info = {
	...
	.sub_module[0] = &cs_ne_ni_port_info[0],
	.sub_module[1] = &cs_ne_ni_port_info[1],
	.sub_module[2] = &cs_ne_ni_port_info[2],
	.sub_module[3] = &cs_ne_ni_port_info[3],
	.sub_module[4] = &cs_ne_ni_port_info[4],
	.sub_module[5] = &cs_ne_ni_port_info[5],
	.sub_module[6] = &cs_ne_ni_port_info[6],
	.sub_module[7] = &cs_ne_ni_port_info[7],
	.sub_module[8] = &cs_ne_ni_rxfifo_info,
	.sub_module[9] = &cs_ne_ni_txem_info,
	.sub_module[10] = &cs_ne_ni_pc_info[0],
	.sub_module[11] = &cs_ne_ni_pc_info[1],
	.sub_module[12] = &cs_ne_ni_pc_info[2],
...
static __u32 global_irq_read(u32 reg);
static void global_irq_write(u32 val, u32 reg);
...
struct cs_ne_irq_info cs_ne_global_irq_info = {
	...
	.read_reg	= global_irq_read,
	.write_reg	= global_irq_write,
};
...
static __u32 global_irq_read(u32 reg)
{
	...
	if (reg == cs_ne_global_irq_info.intr_e_reg)
		val = GLOBAL_READL(reg);
	else
		for (ii = 0 ; ii < 32 ; ii++) {
			if (reg == cs_ne_global_irq_info.sub_intr_reg[ii]) {
				switch (ii) {
				case 0:
				case 1:
				case 2:
...
{
	int i = 0;
	irq_module->write_reg(irq_module->intr_enable, irq_module->intr_e_reg);
	while (i<32) {
...
int cs_ne_schcnfg_intr_handle(u32 dev_id,
			      ...
			      u32 intr_status)
{
	DBG(printk(KERN_INFO "%s(%s)::intr status 0x%08x\n",
		   __func__, irq_module->module_name, intr_status));
	return 0;
}
			     ...
			     u32 intr_status)
{
	...
	while (i < 32) {
		if ((intr_status & flag & irq_module->intr_enable)
			...
			status = irq_module->read_reg(irq_module->sub_intr_reg[i]);
			irq_module->write_reg(status, irq_module->sub_intr_reg[i]);
