...
void cs_qm_init_cfg(void)
{
	...
	if(use_internal_buff == 1){
		...
		QM_WRITE_BITS(qm_int_buf_cfg0.wrd, qm_int_buf_cfg0_mask.wrd, QM_INT_BUF_CONFIG_0);
	...
	QM_WRITE_BITS(qm_cfg0.wrd, qm_cfg0_mask.wrd, QM_CONFIG_0);
...
#define QM_MEM_CFG_1GB		3
	...
	mem_sz = GLOBAL_READL(GLOBAL_SOFTWARE2) >> 20;
        switch(mem_sz){
        case 128:
	...
	QM_WRITE_BITS(qm_cfg1.wrd, qm_cfg1_mask.wrd, QM_CONFIG_1);

	/* ACP for QM */
#if 0
	GLOBAL_WRITEL(0x0000001F, GLOBAL_ARM_CONFIG_E);
	 /* ACP remap register */
	mem_sz = GLOBAL_READL(GLOBAL_SOFTWARE2) >> 20;
...
void cs_qm_init(void)
{
	...
	do {
		init_done = (QM_READL(QM_STATUS_0) & 0x1);
		/*printk("init_done = %d\n",init_done);*/
		udelay(10);
	} while (init_done == 0);
...
void cs_qm_init_cpu_path_cfg(void)
{
	...
	QM_WRITE_BITS(cpu_stat0_cfg.wrd, cpu_stat0_cfg_mask.wrd, QM_CPU_PATH_CONFIG_0);
	...
	QM_WRITE_BITS(cpu_stat1_cfg.wrd, cpu_stat1_cfg_mask.wrd, QM_CPU_PATH_CONFIG_1);
	/*
	...
	for (i = 0; i < 8; i++) {
		voq_map = i | (i << 3) | (i << 6) | (i << 9) | (i << 12) |
		    (i << 15) | (i << 18) | (i << 21);
		QM_WRITEL(voq_map, QM_CPU_PATH_VOQ_MAP_0 + (i * 4));
	}
}
...
{
	int i;
	QM_WRITEL(cfg->cfg_0.wrd, QM_CONFIG_0);
	QM_WRITEL(cfg->cfg_1.wrd, QM_CONFIG_1);

	QM_WRITEL(cfg->cpu_path_cfg_0.wrd, QM_CPU_PATH_CONFIG_0);
	QM_WRITEL(cfg->cpu_path_cfg_1.wrd, QM_CPU_PATH_CONFIG_1);
	 ...
	 */
	for (i = 0; i < 8; i++) {
		QM_WRITEL(QM_CPU_PATH_VOQ_MAP_VAL, qm_voq_map_reg[i]);
	}
	return 0;
}
...
int cs_qm_get_gbl_params(cs_qm_dev_cfg_t * cfg)
{
	if (cfg == NULL)
		return CS_ERROR;
	cfg->cfg_0.wrd = QM_READL(QM_CONFIG_0);
	cfg->cfg_1.wrd = QM_READL(QM_CONFIG_1);
	cfg->cpu_path_cfg_0.wrd = QM_READL(QM_CPU_PATH_CONFIG_0);
	cfg->cpu_path_cfg_1.wrd = QM_READL(QM_CPU_PATH_CONFIG_1);
...
{
	QM_WRITEL(buf_addr, qm_linuxq_sdram_reg[linux_qid & 0x07]);
	return 0;
}
...
int cs_qm_get_free_cpu_buffer_cnt(u32 * cnt)
{
	...
	status_0.wrd = QM_READL(QM_CPU_PATH_STATUS_0);
...
{
	u32 buf_cnt;
	buf_cnt = QM_READL(qm_cpu_free_buffers[qid][0]);
...
int cs_qm_get_cpu_buffer_cnt(u32 qid, u32 * cnt)
{
	...
	buffers.wrd = QM_READL(qm_cpu_path_buffers[idx]);
	if (low == 0)
		*cnt = buffers.bf.low;
	else
		*cnt = buffers.bf.high;
...
{
	reg->wrd = QM_READL(QM_CPU_BUFFER_LIST_MEM_STATUS);
	return 0;
}

int cs_qm_get_profile_mem_status(QM_PROFILE_MEM_STATUS_t * reg)
{
	reg->wrd = QM_READL(QM_PROFILE_MEM_STATUS);
	return 0;
}

int cs_qm_get_status_mem_status(QM_STATUS_MEM_STATUS_t * reg)
{
	reg->wrd = QM_READL(QM_STATUS_MEM_STATUS);
	return 0;
}
...
int cs_qm_set_profile_mem(unsigned char addr, cs_qm_profile_mem_t *profile_mem)
{
	...
	QM_WRITEL(profile_mem->wrd, QM_QUE_PROFILE_MEM_DATA);
	QM_WRITEL(qm_profile_access.wrd, QM_QUE_PROFILE_MEM_ACCESS);
	if (cs_qm_wait_access_done(QM_QUE_PROFILE_MEM_ACCESS) == -1)
		return -1;
