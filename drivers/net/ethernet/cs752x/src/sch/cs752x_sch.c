...
static int sch_cpu_start_write(void)
{
	...
	SCH_WRITEL(cmd_control.wrd, SCH_ATOMIC_COMMAND_CONTROL);

	/* Wait for interrupt status to be set. */
	do {
		cpu_cmd_executed = (SCH_READL(SCH_INTERRUPT_0) & 0x1);
		udelay(1);
	} while ((cpu_cmd_executed == 0) && timeout--);

	/* Clear interrupt */
	if (cpu_cmd_executed != 0)
		SCH_WRITEL(cpu_cmd_executed, SCH_INTERRUPT_0);
	...
	status.wrd = SCH_READL(SCH_STATUS);
		...
		sched_id_t * p_cal_entries)
{
	...
	if (tdm_id == MAIN_TDM_A) {
		SCH_WRITEL(cal0, SCH_MAIN_TDM_PORT_CALENDAR0_SETA);
		SCH_WRITEL(cal1, SCH_MAIN_TDM_PORT_CALENDAR1_SETA);
		SCH_WRITEL((entry_num - 1), SCH_MAIN_TDM_CONTROL_SETA);
	} else {
		SCH_WRITEL(cal0, SCH_MAIN_TDM_PORT_CALENDAR0_SETB);
		SCH_WRITEL(cal1, SCH_MAIN_TDM_PORT_CALENDAR1_SETB);
		SCH_WRITEL((entry_num - 1), SCH_MAIN_TDM_CONTROL_SETB);
	}
				   ...
				   unsigned char cal_len)
{
	...
	if (tdm_id == MAIN_TDM_A) {
		cal0 = SCH_READL(SCH_MAIN_TDM_PORT_CALENDAR0_SETA);
		cal1 = SCH_READL(SCH_MAIN_TDM_PORT_CALENDAR1_SETA);
	} else {
		cal0 = SCH_READL(SCH_MAIN_TDM_PORT_CALENDAR0_SETB);
		cal1 = SCH_READL(SCH_MAIN_TDM_PORT_CALENDAR1_SETB);
	}
...
static int sch_change_port_mask(unsigned char port_id, unsigned char mask)
{
	...
	SCH_WRITEL(cmd_data0.wrd, SCH_ATOMIC_COMMAND_DATA0);
	SCH_WRITEL(cmd_data1.wrd, SCH_ATOMIC_COMMAND_DATA1);
...
static int sch_reset(void)
{
	...
	/* Soft Reset */
	SCH_WRITEL(1, SCH_CONTROL);
	udelay(100);

	/* Clear Reset */
	SCH_WRITEL(0, SCH_CONTROL);
	udelay(100);
...
int cs752x_sch_get_curr_tdm_id(unsigned char * p_tdm_id)
{
	...
	*p_tdm_id = (SCH_READL(SCH_STATUS) & 0x2) > 1;
...
{
	SCH_WRITEL(1, SCH_SWITCH_TDM_CONTROL_SET);	/* Switch TDM */
		...
		unsigned int quanta, unsigned int bandwidth)
{
	...
	clock_per_round.wrd = SCH_READL(SCH_SHAPER_CONFIGURATION);
...
//		}
	...
	sch_shaper_cfg.wrd = SCH_READL(SCH_SHAPER_CONFIGURATION);
	sch_shaper_cfg.bf.global_scrub_enable = 1;
	sch_shaper_cfg.bf.clocks_per_round_st = 0x100;
	SCH_WRITEL(sch_shaper_cfg.wrd, SCH_SHAPER_CONFIGURATION);

	sch_dbg_ctrl.wrd = SCH_READL(SCH_DEBUG_CONTROL);
	sch_dbg_ctrl.bf.switch_expr_mode_lowlvl = 3;
	SCH_WRITEL(sch_dbg_ctrl.wrd, SCH_DEBUG_CONTROL);
