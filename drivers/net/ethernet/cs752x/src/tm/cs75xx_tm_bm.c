...
int cs_tm_bm_set_cfg(cs_tm_bm_cfg_t *cfg)
{
	...
	status.wrd = TM_READL(TM_BM_STATUS_0);
	if (status.bf.init_done == 0)
		return -1;

	bm_cfg.wrd = TM_READL(TM_BM_CONFIG_0);
	...
	TM_WRITEL(bm_cfg.wrd,TM_BM_CONFIG_0);
...
int cs_tm_bm_get_free_buf_info(unsigned int *total_buf, unsigned int *free_buf)
{
	...
	status.wrd = TM_READL(TM_BM_STATUS_1);
		...
		unsigned int *free_buf)
{
	...
	status.wrd = TM_READL(TM_BM_STATUS_2);
...
int cs_tm_bm_set_reserve_buffers(cs_tm_bm_reserve_buffer_t *rsv_buf)
{
	...
	TM_WRITEL(bufs.wrd,TM_BM_RESERVE_BUFFERS);
	TM_WRITEL(cpu_bufs.wrd,TM_BM_RESERVE_CPU_BUFFERS);
	TM_WRITEL(linux_bufs_0.wrd,TM_BM_RESERVE_LINUX_BUFFERS_0);
	TM_WRITEL(linux_bufs_1.wrd,TM_BM_RESERVE_LINUX_BUFFERS_1);
	TM_WRITEL(linux_bufs_2.wrd,TM_BM_RESERVE_LINUX_BUFFERS_2);
	TM_WRITEL(linux_bufs_3.wrd,TM_BM_RESERVE_LINUX_BUFFERS_3);
	return 0;
}
...
int cs_tm_bm_get_reserve_buffers(cs_tm_bm_reserve_buffer_t *rsv_buf)
{
	...
	bufs.wrd = TM_READL(TM_BM_RESERVE_BUFFERS);
	cpu_bufs.wrd = TM_READL(TM_BM_RESERVE_CPU_BUFFERS);
	linux_bufs_0.wrd = TM_READL(TM_BM_RESERVE_LINUX_BUFFERS_0);
	linux_bufs_1.wrd = TM_READL(TM_BM_RESERVE_LINUX_BUFFERS_1);
	linux_bufs_2.wrd = TM_READL(TM_BM_RESERVE_LINUX_BUFFERS_2);
	linux_bufs_3.wrd = TM_READL(TM_BM_RESERVE_LINUX_BUFFERS_3);
...
int cs_tm_bm_set_wred_cfg(cs_tm_bm_wred_cfg_t *cfg)
{
	...
	TM_WRITEL(wred_cfg_0.wrd, TM_BM_WRED_CONFIG_0);
	TM_WRITEL(wred_cfg_1.wrd, TM_BM_WRED_CONFIG_1);
	return 0;
}
...
int cs_tm_bm_get_wred_cfg(cs_tm_bm_wred_cfg_t *cfg)
{
	...
	wred_cfg_0.wrd = TM_READL(TM_BM_WRED_CONFIG_0);
	wred_cfg_1.wrd = TM_READL(TM_BM_WRED_CONFIG_1);
...
int cs_tm_bm_set_copy_wred_profile(cs_tm_bm_wred_profile_t *profile)
{
	...
	TM_WRITEL(wred_profile.wrd, TM_BM_COPY_WRED_PROFILE);
	return 0;
}
...
int cs_tm_bm_get_copy_wred_profile(cs_tm_bm_wred_profile_t *profile)
{
	...
	wred_profile.wrd = TM_READL(TM_BM_COPY_WRED_PROFILE);
	profile->wred_aqd_min_pct_baseline =
		wred_profile.bf.wred_aqd_min_pct_baseline;
	profile->wred_aqd_max_pct_baseline =
		wred_profile.bf.wred_aqd_max_pct_baseline;
...
int cs_tm_bm_set_copy_dest_port(unsigned char port)
{
	...
	TM_WRITEL(dest_port.wrd, TM_BM_COPY_DEST_PORT);
	return 0;
}
...
int cs_tm_bm_get_copy_dest_port(unsigned char *port)
{
	...
	dest_port.wrd = TM_READL(TM_BM_COPY_DEST_PORT);
...
int cs_tm_bm_set_traffic_control(cs_tm_bm_traffic_control_t *tc_cfg)
{
	...
	TM_WRITEL(tc0.wrd, TM_BM_TRAFFIC_CONTROL_0);
	TM_WRITEL(tc1.wrd, TM_BM_TRAFFIC_CONTROL_1);
	return 0;
}
...
int cs_tm_bm_get_traffic_control(cs_tm_bm_traffic_control_t *tc_cfg)
{
	...
	tc0.wrd = TM_READL(TM_BM_TRAFFIC_CONTROL_0);
	tc1.wrd = TM_READL(TM_BM_TRAFFIC_CONTROL_1);
...
int cs_tm_bm_set_traffic_control_int(cs_tm_bm_traffic_control_t *tc_cfg)
{
	...
	TM_WRITEL(tc2.wrd, TM_BM_TRAFFIC_CONTROL_2);
	TM_WRITEL(tc3.wrd, TM_BM_TRAFFIC_CONTROL_3);
	return 0;
}
...
int cs_tm_bm_get_traffic_control_int(cs_tm_bm_traffic_control_t *tc_cfg)
{
	...
	tc2.wrd = TM_READL(TM_BM_TRAFFIC_CONTROL_2);
	tc3.wrd = TM_READL(TM_BM_TRAFFIC_CONTROL_3);
		...
		unsigned char *err_addr, unsigned char *err_correct_addr)
{
	...
	case CS_TM_BM_DEST_PORT_STATUS_MEM:
		dest_port_status_mem.wrd =
			TM_READL(TM_BM_DEST_PORT_STATUS_MEM_STATUS);
...
int cs_tm_bm_get_voq_mem(unsigned char addr, cs_tm_bm_voq_mem_t *voq)
{
	...
	TM_WRITEL(voq_access.wrd, TM_BM_VOQ_MEM_ACCESS);
	if (cs_tm_bm_wait_access_done(TM_BM_VOQ_MEM_ACCESS) == -1)
		return (-1);


	voq_data.wrd = TM_READL(TM_BM_VOQ_MEM_DATA);
...
int cs_tm_bm_set_voq_mem(unsigned char addr, cs_tm_bm_voq_mem_t *voq)
{
	...
	TM_WRITEL(voq_data.wrd, TM_BM_VOQ_MEM_DATA);
		...
		cs_tm_bm_voq_status_mem_t *voq_status)
{
	...
	TM_WRITEL(voq_access.wrd, TM_BM_VOQ_STATUS_MEM_ACCESS);
	if (cs_tm_bm_wait_access_done(TM_BM_VOQ_STATUS_MEM_ACCESS) == -1)
		return (-1);

	voq_data0.wrd = TM_READL(TM_BM_VOQ_STATUS_MEM_DATA0);
	voq_data1.wrd = TM_READL(TM_BM_VOQ_STATUS_MEM_DATA1);
		...
		cs_tm_bm_voq_status_mem_t *voq_status)
{
	...
	TM_WRITEL(voq_data0.wrd, TM_BM_VOQ_STATUS_MEM_DATA0);
	TM_WRITEL(voq_data1.wrd, TM_BM_VOQ_STATUS_MEM_DATA1);
		...
		cs_tm_bm_dest_port_mem_t *dest_port)
{
	...
	TM_WRITEL(dest_port_access.wrd, TM_BM_DEST_PORT_MEM_ACCESS);
	if (cs_tm_bm_wait_access_done(TM_BM_DEST_PORT_MEM_ACCESS) == -1)
		return (-1);

	dest_port_data.wrd = TM_READL(TM_BM_DEST_PORT_MEM_DATA);
		...
		cs_tm_bm_dest_port_mem_t *dest_port)
{
	...
	TM_WRITEL(dest_port_data.wrd, TM_BM_DEST_PORT_MEM_DATA);
		...
		unsigned short *dest_port_global_buffers)
{
	...
	TM_WRITEL(dest_port_access.wrd, TM_BM_DEST_PORT_STATUS_MEM_ACCESS);
	if (cs_tm_bm_wait_access_done(TM_BM_DEST_PORT_STATUS_MEM_ACCESS) == -1)
		return (-1);

	dest_port_data.wrd = TM_READL(TM_BM_DEST_PORT_STATUS_MEM_DATA);
		...
		unsigned short dest_port_global_buffers)
{
	...
	TM_WRITEL(dest_port_data.wrd, TM_BM_DEST_PORT_STATUS_MEM_DATA);
		...
		cs_tm_bm_voq_profile_mem_t *voq_profile)
{
	...
	TM_WRITEL(voq_profile_access.wrd, TM_BM_VOQ_PROFILE_MEM_ACCESS);
	if (cs_tm_bm_wait_access_done(TM_BM_VOQ_PROFILE_MEM_ACCESS) == -1)
		return (-1);

	voq_profile_data0.wrd = TM_READL(TM_BM_VOQ_PROFILE_MEM_DATA0);
	voq_profile_data1.wrd = TM_READL(TM_BM_VOQ_PROFILE_MEM_DATA1);
		...
		cs_tm_bm_voq_profile_mem_t *voq_profile)
{
	...
	TM_WRITEL(voq_profile_data0.wrd, TM_BM_VOQ_PROFILE_MEM_DATA0);
	TM_WRITEL(voq_profile_data1.wrd, TM_BM_VOQ_PROFILE_MEM_DATA1);
		...
		cs_tm_bm_wred_profile_t *wred_profile)
{
	...
	TM_WRITEL(wred_profile_access.wrd, TM_BM_WRED_PROFILE_MEM_ACCESS);
	if (cs_tm_bm_wait_access_done(TM_BM_WRED_PROFILE_MEM_ACCESS) == -1)
		return (-1);

	wred_profile_data.wrd = TM_READL(TM_BM_WRED_PROFILE_MEM_DATA);
		...
		cs_tm_bm_wred_profile_t *wred_profile)
{
	...
	TM_WRITEL(wred_profile_data.wrd, TM_BM_WRED_PROFILE_MEM_DATA);
