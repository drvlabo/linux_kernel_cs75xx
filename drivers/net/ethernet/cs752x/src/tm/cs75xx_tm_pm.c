...
int cs_tm_pm_set_cfg(cs_tm_pm_cfg_t *cfg)
{
	...
	status.wrd = TM_READL(TM_PM_STATUS_0);
	if (status.bf.init_done == 0)
		return -1;

	pm_cfg.wrd = TM_READL(TM_PM_CONFIG_0);
	...
	TM_WRITEL(pm_cfg.wrd, TM_PM_CONFIG_0);
	return 0;
}
		...
		unsigned short *err_addr)
{
	...
	status.wrd = TM_READL(TM_PM_CNTR_MEM_STATUS);
...
int cs_tm_pm_get_glb_cntr_mem_status(unsigned int *err_addr)
{
	...
	status.wrd = TM_READL(TM_PM_GLB_CNTR_MEM_STATUS);
	*err_addr = status.bf.err_addr;
	return 0;
}
		...
		void *counter)
{
	...
	for (i = start_adr; i < (start_adr + size); i++) {
		...
		TM_WRITEL(access.wrd, TM_PM_CNTR_MEM_ACCESS);
		if (cs_tm_pm_wait_access_done(TM_PM_CNTR_MEM_ACCESS) == -1)
			return (-1);

		data0.wrd = TM_READL(TM_PM_CNTR_MEM_DATA0);
		data1.wrd = TM_READL(TM_PM_CNTR_MEM_DATA1);
		data2.wrd = TM_READL(TM_PM_CNTR_MEM_DATA2);
		data3.wrd = TM_READL(TM_PM_CNTR_MEM_DATA3);
		data4.wrd = TM_READL(TM_PM_CNTR_MEM_DATA4);
		data5.wrd = TM_READL(TM_PM_CNTR_MEM_DATA5);
		data6.wrd = TM_READL(TM_PM_CNTR_MEM_DATA6);
...
static int cs_tm_pm_reset_cntr_mem(unsigned int start_adr, unsigned int size)
{
	...
	for (i = start_adr; i < (start_adr + size); i++) {
		TM_WRITEL(0, TM_PM_CNTR_MEM_DATA0);
		TM_WRITEL(0, TM_PM_CNTR_MEM_DATA1);
		TM_WRITEL(0, TM_PM_CNTR_MEM_DATA2);
		TM_WRITEL(0, TM_PM_CNTR_MEM_DATA3);
		TM_WRITEL(0, TM_PM_CNTR_MEM_DATA4);
		TM_WRITEL(0, TM_PM_CNTR_MEM_DATA5);
		TM_WRITEL(0, TM_PM_CNTR_MEM_DATA6);
...
static int cs_tm_pm_get_glb_cntr_mem(cs_tm_pm_glb_blk_t *counter)
{
	...
	for (i = CS_TM_PM_GLB_RX_ALL; i <= CS_TM_PM_GLB_CE; i++) {
		...
		access.bf.addr = i;
		access.bf.access = CS_ENABLE;
		TM_WRITEL(access.wrd, TM_PM_GLB_CNTR_MEM_ACCESS);
		if (cs_tm_pm_wait_access_done(TM_PM_GLB_CNTR_MEM_ACCESS) == -1)
			return (-1);

		data0.wrd = TM_READL(TM_PM_GLB_CNTR_MEM_DATA0);
		data1.wrd = TM_READL(TM_PM_GLB_CNTR_MEM_DATA1);
		data2.wrd = TM_READL(TM_PM_GLB_CNTR_MEM_DATA2);
