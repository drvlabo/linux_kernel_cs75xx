...
int cs_tm_pol_set_cfg(cs_tm_pol_cfg_t *cfg)
{
	...
	status.wrd = TM_READL(TM_POL_STATUS_0);
	if (status.bf.init_done == 0)
		return -1;

	pol_cfg.wrd = TM_READL(TM_POL_CONFIG_0);
	...
	TM_WRITEL(pol_cfg.wrd, TM_POL_CONFIG_0);
...
int cs_tm_pol_set_spid_cfg(cs_tm_pol_cfg_spid_t *cfg)
{
	...
	spid_cfg.wrd = TM_READL(TM_POL_CONFIG_SPID);
	...
	TM_WRITEL(spid_cfg.wrd, TM_POL_CONFIG_SPID);
...
int cs_tm_pol_set_flow_cfg(cs_tm_pol_cfg_flow_t *cfg)
{
	...
	flow_cfg.wrd = TM_READL(TM_POL_CONFIG_FLOW);
	...
	TM_WRITEL(flow_cfg.wrd, TM_POL_CONFIG_FLOW);
...
int cs_tm_pol_set_pkt_type_cfg(cs_tm_pol_cfg_pkt_type_t *cfg)
{
	...
	pkt_type_cfg.wrd = TM_READL(TM_POL_CONFIG_PKT_TYPE);
	...
	TM_WRITEL(pkt_type_cfg.wrd, TM_POL_CONFIG_PKT_TYPE);
...
int cs_tm_pol_set_cpu_cfg(cs_tm_pol_cfg_cpu_t *cfg)
{
	...
	cpu_cfg.wrd = TM_READL(TM_POL_CONFIG_CPU);
	...
	TM_WRITEL(cpu_cfg.wrd, TM_POL_CONFIG_CPU);
...
int cs_tm_pol_set_ipq_dest(unsigned char index, unsigned char ipq_val)
{
	...
	dest_ipg.wrd = TM_READL(TM_POL_IPG_DEST);
	dest_ipg.wrd = dest_ipg.wrd | (ipq_val << ((index - 1) * 5));
	TM_WRITEL(dest_ipg.wrd, TM_POL_IPG_DEST);
...
int cs_tm_pol_set_ipq_dest_map(unsigned char port_id, unsigned char index)
{
	...
	dest_ipg_map.wrd = TM_READL(TM_POL_IPG_DEST_MAP);
	dest_ipg_map.wrd = dest_ipg_map.wrd | (index << (port_id * 3));
	TM_WRITEL(dest_ipg_map.wrd, TM_POL_IPG_DEST_MAP);
...
int cs_tm_pol_set_ipq_src(unsigned char index, unsigned char ipq_val)
{
	...
	src_ipg.wrd = TM_READL(TM_POL_IPG_SRC);
	src_ipg.wrd = src_ipg.wrd | (ipq_val << ((index - 1) * 5));
	TM_WRITEL(src_ipg.wrd, TM_POL_IPG_SRC);
...
int cs_tm_pol_set_ipq_src_map(unsigned char port_id, unsigned char index)
{
	...
	src_ipg_map.wrd = TM_READL(TM_POL_IPG_SRC_MAP);
	src_ipg_map.wrd = src_ipg_map.wrd | (index << (port_id * 3));
	TM_WRITEL(src_ipg_map.wrd, TM_POL_IPG_SRC_MAP);
		...
		unsigned char *err_addr, unsigned char *err_correct_addr)
{
	...
	case CS_TM_POL_PKT_TYPE_PROFILE_MEM:
		pkt_type_profile.wrd =
			TM_READL(TM_POL_PKT_TYPE_PROFILE_MEM_STATUS);
	...
	case CS_TM_POL_PKT_TYPE_STATUS_MEM:
		pkt_type_status.wrd = TM_READL(TM_POL_PKT_TYPE_STATUS_MEM_STATUS);
		...
		cs_tm_pol_status_mem_t *bucket)
{
	...
	case CS_TM_POL_FLOW_STATUS_MEM:
		...
		TM_WRITEL(flow_access.wrd, TM_POL_FLOW_STATUS_MEM_ACCESS);
		if (cs_tm_pol_wait_access_done(
			TM_POL_FLOW_STATUS_MEM_ACCESS) == -1)
			return (-1);

		/* read status memory */
		reg_data0 = TM_READL(TM_POL_FLOW_STATUS_MEM_DATA0);
		reg_data1 = TM_READL(TM_POL_FLOW_STATUS_MEM_DATA1);
		break;
	...
	case CS_TM_POL_SPID_STATUS_MEM:
		...
		TM_WRITEL(spid_access.wrd, TM_POL_SPID_STATUS_MEM_ACCESS);
		if (cs_tm_pol_wait_access_done(
			TM_POL_SPID_STATUS_MEM_ACCESS) == -1)
			return (-1);

		/* read status memory */
		reg_data0 = TM_READL(TM_POL_SPID_STATUS_MEM_DATA0);
		reg_data1 = TM_READL(TM_POL_SPID_STATUS_MEM_DATA1);
		break;
	...
	case CS_TM_POL_CPU_STATUS_MEM:
		...
		TM_WRITEL(cpu_access.wrd, TM_POL_CPU_STATUS_MEM_ACCESS);
		if (cs_tm_pol_wait_access_done(
			TM_POL_CPU_STATUS_MEM_ACCESS) == -1)
			return (-1);

		/* read status memory */
		reg_data0 = TM_READL(TM_POL_CPU_STATUS_MEM_DATA0);
		reg_data1 = TM_READL(TM_POL_CPU_STATUS_MEM_DATA1);
		break;
	...
	case CS_TM_POL_PKT_TYPE_STATUS_MEM:
		...
		TM_WRITEL(pkt_type_access.wrd, TM_POL_PKT_TYPE_STATUS_MEM_ACCESS);
		if (cs_tm_pol_wait_access_done(
			TM_POL_PKT_TYPE_STATUS_MEM_ACCESS) == -1)
			return (-1);

		/* read status memory */
		reg_data0 = TM_READL(TM_POL_PKT_TYPE_STATUS_MEM_DATA0);
		reg_data1 = TM_READL(TM_POL_PKT_TYPE_STATUS_MEM_DATA1);
		break;
		...
		cs_tm_pol_status_mem_t *bucket)
{
	...
	switch (type) {
	case CS_TM_POL_FLOW_STATUS_MEM:
		TM_WRITEL(reg_data0, TM_POL_FLOW_STATUS_MEM_DATA0);
		TM_WRITEL(reg_data1, TM_POL_FLOW_STATUS_MEM_DATA1);
	...
	case CS_TM_POL_SPID_STATUS_MEM:
		TM_WRITEL(reg_data0, TM_POL_SPID_STATUS_MEM_DATA0);
		TM_WRITEL(reg_data1, TM_POL_SPID_STATUS_MEM_DATA1);
	...
	case CS_TM_POL_CPU_STATUS_MEM:
		TM_WRITEL(reg_data0, TM_POL_CPU_STATUS_MEM_DATA0);
		TM_WRITEL(reg_data1, TM_POL_CPU_STATUS_MEM_DATA1);
	...
	case CS_TM_POL_PKT_TYPE_STATUS_MEM:
		TM_WRITEL(reg_data0, TM_POL_PKT_TYPE_STATUS_MEM_DATA0);
		TM_WRITEL(reg_data1, TM_POL_PKT_TYPE_STATUS_MEM_DATA1);
...
//	TM_POL_PKT_TYPE_PROFILE_MEM_DATA1_t pkt_type_data1;
	...
	case CS_TM_POL_FLOW_PROFILE_MEM:
		...
		flow_access.bf.access = CS_ENABLE;
		flow_access.bf.rbw = CS_OP_READ;
		TM_WRITEL(flow_access.wrd, TM_POL_FLOW_PROFILE_MEM_ACCESS);
		if (cs_tm_pol_wait_access_done(
			TM_POL_FLOW_PROFILE_MEM_ACCESS) == -1)
			return (-1);

		/* read profile memory */
		reg_data0 = TM_READL(TM_POL_FLOW_PROFILE_MEM_DATA0);
		reg_data1 = TM_READL(TM_POL_FLOW_PROFILE_MEM_DATA1);
		break;
	...
	case CS_TM_POL_SPID_PROFILE_MEM:
		...
		spid_access.bf.access = CS_ENABLE;
		spid_access.bf.rbw = CS_OP_READ;
		TM_WRITEL(spid_access.wrd, TM_POL_SPID_PROFILE_MEM_ACCESS);
		if (cs_tm_pol_wait_access_done(
			TM_POL_SPID_PROFILE_MEM_ACCESS) == -1)
			return (-1);

		/* read profile memory */
		reg_data0 = TM_READL(TM_POL_SPID_PROFILE_MEM_DATA0);
		reg_data1 = TM_READL(TM_POL_SPID_PROFILE_MEM_DATA1);
		//??????????????
		break;
	...
	case CS_TM_POL_CPU_PROFILE_MEM:
		...
		cpu_access.bf.access = CS_ENABLE;
		cpu_access.bf.rbw = CS_OP_READ;
		TM_WRITEL(cpu_access.wrd, TM_POL_CPU_PROFILE_MEM_ACCESS);
		if (cs_tm_pol_wait_access_done(
			TM_POL_CPU_PROFILE_MEM_ACCESS) == -1)
			return (-1);

		/* read profile memory */
		reg_data0 = TM_READL(TM_POL_CPU_PROFILE_MEM_DATA0);
		reg_data1 = TM_READL(TM_POL_CPU_PROFILE_MEM_DATA1);
		break;
	...
	case CS_TM_POL_PKT_TYPE_PROFILE_MEM:
		...
		pkt_type_access.bf.access = CS_ENABLE;
		pkt_type_access.bf.rbw = CS_OP_READ;
		TM_WRITEL(pkt_type_access.wrd, TM_POL_PKT_TYPE_PROFILE_MEM_ACCESS);
		if (cs_tm_pol_wait_access_done(
			TM_POL_PKT_TYPE_PROFILE_MEM_ACCESS) == -1)
			return (-1);

		/* read profile memory */
		reg_data0 = TM_READL(TM_POL_PKT_TYPE_PROFILE_MEM_DATA0);
		reg_data1 = TM_READL(TM_POL_PKT_TYPE_PROFILE_MEM_DATA1);
		break;
...
//	void data1;
	...
	case CS_TM_POL_FLOW_PROFILE_MEM:
		...
		TM_WRITEL(flow_data0.wrd, TM_POL_FLOW_PROFILE_MEM_DATA0);
		TM_WRITEL(flow_data1.wrd, TM_POL_FLOW_PROFILE_MEM_DATA1);
	...
	case CS_TM_POL_SPID_PROFILE_MEM:
		...
		TM_WRITEL(spid_data0.wrd, TM_POL_SPID_PROFILE_MEM_DATA0);
		TM_WRITEL(spid_data1.wrd, TM_POL_SPID_PROFILE_MEM_DATA1);
	...
	case CS_TM_POL_CPU_PROFILE_MEM:
		...
		TM_WRITEL(cpu_data0.wrd, TM_POL_CPU_PROFILE_MEM_DATA0);
		TM_WRITEL(cpu_data1.wrd, TM_POL_CPU_PROFILE_MEM_DATA1);
	...
	case CS_TM_POL_PKT_TYPE_PROFILE_MEM:
		...
		TM_WRITEL(pkt_type_data0.wrd, TM_POL_PKT_TYPE_PROFILE_MEM_DATA0);
		TM_WRITEL(pkt_type_data1.wrd, TM_POL_PKT_TYPE_PROFILE_MEM_DATA1);
